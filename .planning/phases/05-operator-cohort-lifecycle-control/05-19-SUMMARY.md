...
> **CORRECTION, 2026-07-30 (05-25).** The carry described above was built correctly and scoped correctly, to the canceled fate only. What 05-19 did NOT do is test the other side of that scope. `05-AUDIT-2.md` entry 8 found it and it is confirmed.
>
> All three dismissal rows 05-19 added to `packages/service/tests/cohort-fate.spec.ts` CANCEL before they dismiss, so `forgetTerminal`'s `record.fate === 'canceled'` condition was only ever entered on its true side. Widening it to `record.fate` (every fate is truthy) would have shipped green. The consequence is not a lost bit but an invented one: `GET /v1/cohort-fate/:id` would have answered `{"canceled":true}` to any anonymous participant of a cohort that merely ran out of time, and the participant console turns that into "The operator canceled this cohort." A lapse would have been presented as a deliberate act, against a named operator, which is the exact inversion of the honesty D-02 and ADR 0017 exist to protect.
>
> The expired leg is covered in 05-25: an advertised cohort is settled WITHOUT a cancel, dismissed through the real gated route, and the anonymous fate read is asserted to still answer not-canceled, with a canceled control beside it, an undismissed expiry isolating the dismissal, and a byte-identity comparison against an id the service never issued. The widening mutation was run and observed RED on the expired rows and GREEN on the canceled one. See `05-25-SUMMARY.md`. Left in place rather than rewritten, so the record shows both what was built and what was never exercised.
