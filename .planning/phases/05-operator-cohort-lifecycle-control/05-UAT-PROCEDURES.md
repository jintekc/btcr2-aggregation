...
Look for the login panel being replaced by the console: the page heading `Operator console` (`packages/web/src/components/operator/OperatorConsole.tsx:130`) with a `Sign out` control to its right (`OperatorConsole.tsx:132`). If you instead see `Operator console is disabled` (`OperatorConsole.tsx:82`), the service booted without `OPERATOR_PASSWORD`; stop and re-boot.
