...
note (2026-08-02): the SC 6 defect recorded in `05-VERIFICATION.md` (on the default mutinynet network a foreign-chain endpoint rendered the unreachable sentence, so the mismatch verdict was unreachable there) was closed by plan 05-29. The expectation for this walk is therefore the mismatch sentence naming BOTH chains, for any wrong-chain host, not only a signet-family one. The hermetic rows cited below are not the proof: this walk still has to see it against a real third-party host.
result: [pending]
...
| 15 | A foreign-chain endpoint on THIS SERVICE'S DEFAULT NETWORK is refused with a mismatch naming BOTH chains, rather than reported as a host that could not be reached | `packages/web/tests/tx-client.spec.ts`, the default-network rows "names bitcoin/testnet3/testnet4/regtest against the DEFAULT network, where no second marker can ever arrive" (one per chain family), "names an unregistered chain honestly on the DEFAULT network too" and "names a mainnet endpoint on the DEFAULT network, in ONE request", with "accepts NOTHING new: every registry pairing, with no marker observed" holding the refusal posture across the whole registry. HERMETIC LIMIT: these produce the verdict against a stubbed endpoint. A real CORS refusal and a real wrong-chain third-party host are NOT covered and stay with test 15 | 05-29 |
